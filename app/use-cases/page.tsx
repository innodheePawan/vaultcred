            ...
            <main className="relative z-10 pt-16">
                ...
                <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                    <div className="max-w-3xl">
                        <div className="text-[11px] font-mono uppercase tracking-wider text-indigo-400/70 mb-4">Use Cases</div>
                ...
                <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto space-y-20">
                    {tier1UseCases.map((uc, idx) => (
                        <div key={idx} className={`border border-white/[0.06] rounded-xl bg-white/[0.02] border-l-2 ${uc.color} overflow-hidden`}>
                            {/* Header */}
                            <div className="px-6 sm:px-8 pt-8 pb-6">
                                <div className="flex items-center gap-3 mb-4">
                                    <uc.icon className="w-5 h-5 text-slate-400" strokeWidth={1.5} />
                                    <h2 className="text-xl font-bold text-white">{uc.title}</h2>
                                </div>

                                {/* Problem */}
                                <div className="mb-6">
                                    <div className="text-[10px] font-mono uppercase tracking-wider text-slate-600 mb-2">The Problem</div>
                                    <p className="text-sm text-slate-400 leading-relaxed">{uc.problem}</p>
                                </div>

                                {/* Pain Points */}
                                <div className="mb-6">
                                    <div className="text-[10px] font-mono uppercase tracking-wider text-slate-600 mb-2">Operational Pain</div>
                                ...
                                {/* Solution */}
                                <div className="mb-6">
                                    <div className="text-[10px] font-mono uppercase tracking-wider text-slate-600 mb-2">How CredSecure Solves It</div>
                                    <p className="text-sm text-slate-400 leading-relaxed">{uc.solution}</p>
                                </div>
                            </div>

                            {/* Workflow */}
                            <div className="px-6 sm:px-8 py-6 border-t border-white/[0.04] bg-white/[0.01]">
                                <div className="text-[10px] font-mono uppercase tracking-wider text-slate-600 mb-4">Governance Workflow</div>
                            ...
                            {/* Outcome */}
                            <div className="px-6 sm:px-8 py-4 border-t border-white/[0.04] bg-white/[0.02]">
                                <div className="flex items-start gap-2">
                                    <div className={`w-1.5 h-1.5 rounded-full ${uc.dotColor} mt-1.5 shrink-0`} />
                                    <p className="text-xs text-slate-400 font-medium">{uc.outcome}</p>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <section className="py-16 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
                    <div className="text-[10px] font-mono uppercase tracking-wider text-slate-600 mb-6">Additional Use Cases</div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {tier2UseCases.map((uc, idx) => (
                            <div key={idx} className="p-5 rounded-lg border border-white/[0.06] bg-white/[0.02] hover:border-white/[0.1] transition-colors">
                                <div className="flex items-center gap-2 mb-3">
                                    <uc.icon className="w-4 h-4 text-slate-500" strokeWidth={1.5} />
